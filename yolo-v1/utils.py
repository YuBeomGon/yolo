...
def get_bboxes(
    ...
    for batch_idx, (x, labels, gbboxes) in enumerate(tqdm(loader)):
        c, h, w = x[0].shape
        x = torch.cat(x).view(-1, c, h, w)
        lh, lw, lc = labels[0].shape
#         print('labels', labels[0].shape )
        labels = torch.cat(labels).view(-1, lh, lw, lc)    
#         print('labels', labels[0].shape )
        x = x.to(device)
...
#         print('bboxes', len(bboxes[0]), len(bboxes[1]))
#         print('true_box', len(true_bboxes[0]),len(true_bboxes[1]))
...
def convert_cellboxes(predictions, S=25):
    """
...
def cellboxes_to_boxes(out, S=25):
