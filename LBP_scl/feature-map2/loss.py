...
# center point bbox regression            
        ab_point_pred = point[:,:,:]
#         ab_cell_pred = self.clamp(ab_cell_pred)
        ab_point_label = labels[:,:,1:6] # x,y, widht, height
        ab_point_loss = []
        for i in range(batch_size) :
...
#                     if not torch.isfinite(self.mse(ab_point_pred[i,j,0:2], ab_point_label[i,j,1:3])) :
                    ...
                    size_loss.append(self.mse(ab_point_pred[i,j,2:4], ab_point_label[i,j,3:5]))
            else :
                point_loss.append(torch.tensor(0.).to(self.device))
                size_loss.append(torch.tensor(0.).to(self.device))
            ab_point_loss.append(torch.stack(point_loss).sum() + torch.stack(size_loss).sum())              

        multi_pred = multi_out[:,:,0]
        multi_pred = self.clamp(multi_pred)
        multi_label = labels[:,:,6]            
        multi_loss = []
        for i in range(batch_size) :
    ...
    5.*torch.stack(multi_loss).mean() + torch.stack(ab_point_loss).mean(), torch.stack(batch_loss).mean(), torch.stack(ab_batch_loss).mean(), torch.stack(multi_loss).mean(), torch.stack(ab_point_loss).mean()
